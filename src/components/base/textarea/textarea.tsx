...
export const TextAreaBase = ({ className, ...props }: TextAreaBaseProps) => {
  return (
    <AriaTextArea
      ...
      className={(state) =>
        cx(
          "w-full scroll-py-3 rounded-md bg-white px-3.5 py-3 text-sm text-gray-800 shadow-xs ring-1 ring-gray-200 transition duration-100 ease-linear ring-inset placeholder:text-gray-400 autofill:rounded-lg autofill:text-blue-600 focus:outline-hidden",

          // Resize handle
          "[&::-webkit-resizer]:bg-(image:--resize-handle-bg) [&::-webkit-resizer]:bg-contain dark:[&::-webkit-resizer]:bg-(image:--resize-handle-bg-dark)",

          state.isFocused && !state.isDisabled && "ring-1 ring-gray-400",
          state.isDisabled &&
